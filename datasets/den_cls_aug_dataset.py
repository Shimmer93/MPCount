...
class DenClsAugDataset(DensityMapDataset):
    def collate(batch):
    ...
    def __init__(self, root, aug_root, crop_size, downsample, method, is_grey, unit_size, pre_resize=1, roi_map_path=None, gt_dir=None, gen_root=None):
    ...
    def __getitem__(self, index):
        ...
        if self.method == 'train':
            ...
            img1, img2, gt, dmap = self._train_transform(img, img_aug, gt, dmap)
            ...
            return self._val_transform(img, img_aug, gt, basename)
    ...
    def _train_transform(self, img, img_aug, gt, dmap):
        w, h = img.size
        assert len(gt) >= 0
        assert img.size == img_aug.size
        ...
        if random.random() > 0.88:
            ...
            img_aug = img_aug.convert('L').convert('RGB')
        ...
        h, w = self.crop_size[0], self.crop_size[1]
        img_aug = F.crop(img_aug, i, j, h, w)
        h, w = self.crop_size[0], self.crop_size[1]
        ...
        img2 = self.more_transform(img_aug)
        gt = torch.from_numpy(gt.copy()).float()
        dmap = dmap.float()

        return img1, img2, gt, dmap

    def _val_transform(self, img, img_aug, gt, name):
        assert img.size == img_aug.size

        if self.pre_resize != 1:
            img = img.resize((int(img.size[0] * self.pre_resize), int(img.size[1] * self.pre_resize)))
            img_aug = img_aug.resize((int(img_aug.size[0] * self.pre_resize), int(img_aug.size[1] * self.pre_resize)))
...
if __name__ == '__main__':
    dataset = DenClsAugDataset('/mnt/home/zpengac/USERDIR/Crowd_counting/datasets/sta', 320, 1, 'train', False, 1)
